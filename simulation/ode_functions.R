...
get_truefd <- function(abc=NULL){
  ...
  for(ii in 1:neuron_num){
    true.list[[ii]] = fd(coef = coeflist[[ii]], basisobj = bs.list[[ii]])
  }
  
  true.int = matrix(NA, nrow = length(timeseq), ncol = neuron_num )
  true.der = matrix(NA, nrow = length(timeseq), ncol = neuron_num )
  
  for (jj in 1:neuron_num){
    true.int[,jj] <- exp(eval.fd(timeseq, true.list[[jj]]))
    true.der[,jj] <- eval.fd(timeseq, true.list[[jj]],1)* exp(eval.fd(timeseq, true.list[[jj]]))
  }
  
  return(list(true.list = true.list, true.int = true.int, true.der = true.der))
}


getbeta <- function(target, basis_list, coef_list, amp, support = c(0,1),
                    sp.order = 4, num.knots = 11, lambda, lambda_I = 100,
                    alpha = 3.7, gamma = 1e5, iter.max = 100,
                    fix.estimation = TRUE, connection){
  #Input:
  ...
  for (iter in 1:iter.max){
    if (iter == 1){
      ...
      if (fix.estimation){
        for (index in 1:12){
          if (!(index %in% connection)){
            loc = ((index - 1)*basis_obj$nbasis+1):(index*basis_obj$nbasis)
            coef_est[loc,iter] = rep(0, basis_obj$nbasis)
          }
        }
      }
    }else{
      ...
      if(length(which(betanonzero)) == 0 || length(which(betanonzero)) == 1){
        print('too sparse')
        
        
        iter = iter.max
        coef_est[,iter.max] = coef_last
...
sim_estimation <- function(whichneuron, coef_sim, lambda_cand,amp_vec,connection_vec){
  
  
  #basis objects for intensity curves
  neuron_num   <- 12
  numberofinterior <- c(6,3,3,3,3,3,3,2,5,4,5,3)
  time.start   <- 0
  time.end     <- 20000
  ...
  simulationresult <- matrix(NA, nrow = basis_obj$nbasis * neuron_num, ncol = ncol(coef_sim))
  penaltyresult    <- rep(NA, ncol(coef_sim))
  
  
  
  for (index in 1:ncol(coef_sim)){
    #replace true coefficients with simulated coefficients
    print(paste('Running on tuning #',index,' of ',ncol(coef_sim),' simulation for neuron ',whichneuron,sep=''))
    coeflist[[whichneuron]]  <- coef_sim[,index]
    neuron_num <- 12
    
    
    for (jj in 1:length(lambda_cand)){
      print(paste('Running on tuning #',jj,' of ',length(lambda_cand),' tuning for ',whichneuron,sep=''))
      temp <- getbeta(target = whichneuron, basis_list = bs.list, amp = amp_vec[whichneuron], coef_list = coeflist,
                      lambda = lambda_cand,alpha = 3.7, lambda_I = 100, gamma =100000, iter.max =150,
                      fix.estimation = TRUE, connection = connection_vec)
      try_result = temp$coef_est
    }
    ...
    #   leveling['change',jk] = (leveling['AICc',jk-1] - leveling['AICc',jk]) /abs(leveling['AICc',jk])
    # }
    # bestindex <- which(leveling['change',] == max(leveling['change',]))
    
    simulationresult[,index] <- try_result
    #penaltyresult[index]     <- lambda_cand[bestindex]
    
  }
  
  # return(list(simulationresult = simulationresult, penaltyresult = penaltyresult))
